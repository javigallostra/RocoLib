...
swaggerui_blueprint = get_swaggerui_blueprint(
    SWAGGER_URL, API_URL, config={"app_name": "RocoLib API"}
)
...
def create_boulder() -> str:
    return render_template(
        "create_boulder.html",
        latest_set=utils.get_show_only_latest_wall_sets(current_user),
    )
...
def explore() -> str:
    return render_template(
        ...
        walls=db_controller.get_gym_walls(
            get_gym(), g.db, utils.get_show_only_latest_wall_sets(current_user)
        ),
    )
...
def explore_boulders() -> str:
    return request_processor.handle_explore_boulders(
        request, session, g.db, current_user
    )


@app.route("/explore_circuits", methods=["GET", "POST"])
def explore_circuits() -> str:
    return request_processor.handle_explore_circuits(
        request, session, g.db, current_user
    )
...
def change_gym_problem_list() -> str:
    return request_processor.handle_change_gym_problem_list_request(
        request, session, g.db, current_user
    )
...
def load_circuit() -> Union[str, NoReturn]:
    return request_processor.process_load_circuit_request(
        request, session, g.db, current_user, app.static_folder
    )
...
def load_next_problem():
    return request_processor.process_load_next_problem_request(
        request, session, g.db, current_user, app.static_folder
    )
...
def load_previous_problem():
    return request_processor.process_load_previous_problem_request(
        request, session, g.db, current_user, app.static_folder
    )
...
def random_problem() -> str:
    return request_processor.process_random_problem_request(
        request, session, g.db, current_user, app.static_folder
    )
...
def wall_section(wall_section) -> str:
    return request_processor.process_wall_section_request(
        request, session, g.db, current_user, app.static_folder, wall_section
    )
...
def circuit_save() -> Response:
    return request_processor.process_save_request(
        request, session, g.db, is_circuit=True
    )
...
def login() -> Union[str, Response]:
    return request_processor.process_login_request(
...
def tick_list() -> Union[str, Response]:
    return request_processor.process_ticklist_request(
        request, session, g.db, current_user
    )
...
def profile() -> Union[str, Response]:
    return request_processor.process_profile_request(
        request, g.db, session, current_user
    )
...
def delete_ticklist_problem() -> Union[Response, NoReturn]:
    return request_processor.process_delete_ticklist_problem_request(
        request, g.db, current_user
    )
